...
// Upload File to R2/B2 via S3 Presigned URL (CORS-compatible)
    ...
    return new Promise((resolve, reject) => {
        ...
        xhr.addEventListener('load', () => {
            if (xhr.status >= 200 && xhr.status < 300) {
                console.log('✅ Upload successful to R2/B2');
                resolve();
            } else {
        ...
        // S3 Presigned URL: Simple PUT with Content-Type
        xhr.open('PUT', uploadData.upload_url);
        xhr.setRequestHeader('Content-Type', uploadData.content_type);

        console.log('📤 Uploading to R2/B2 via S3 presigned URL:', {
            method: 'PUT',

...
- ✅ **Champs réutilisables tels quels** : `order_id`, `product_id`, `buyer_user_id`, `seller_user_id`, `crypto_currency`, `payment_status`, `created_at`
...
- ❌ **Champs OBSOLÈTES** : `product_title` (JOIN avec products), `payment_id`, `payment_currency`, `crypto_amount`, `payment_address`, `nowpayments_id`, `seller_revenue`, `file_delivered`, `download_count`, `last_download_at`
...
- `nowpayments_id`, `payment_id`, `payment_address`, `crypto_amount` → déplacés vers table `payment_transactions` (si nécessaire)
